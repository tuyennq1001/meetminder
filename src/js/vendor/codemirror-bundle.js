...
var TaskCheckboxWidget = class extends WidgetType {
  constructor(checked, from, to, readOnly2 = false) {
    super();
...
var smartListKeymap = [
  {
    ...
    run: (view) => {
      ...
      const line = doc2.lineAt(main.head);
      const lineText = line.text;
      ...
      if (/^\d+\.$/.test(bullet)) {
        ...
        nextPrefix = `${indent}${num}.${space4}`;
      } else if (taskMarker) {
        nextPrefix = `${indent}${bullet} [ ] `;
      } else {
        nextPrefix = `${indent}${bullet}${space4}`;
      }
      dispatch({
...
${nextPrefix}` },
  ...
  },
  {
    ...
    run: (view) => {
      ...
      if (match || !main.empty) {
        const startLine = doc2.lineAt(main.from).number;
        const endLine = doc2.lineAt(main.to).number;
        const changes = [];
        for (let l = startLine; l <= endLine; l++) {
          const lineObj = doc2.line(l);
...
var meetMinderDarkTheme = EditorView.theme(
  {
    "&": {
      color: "#f4f4f5",
...
var NotesEditor = class {
  ...
  setReadOnly(readOnly2) {
    if (!this.view) return;
    this.view.dispatch({
      effects: this.readOnlyCompartment.reconfigure(EditorState.readOnly.of(readOnly2))
    });
  }
  isReadOnly() {
